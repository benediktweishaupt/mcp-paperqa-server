...
/**
 * Tool validation utility class
 * Validates tool registrations and parameter schemas
 */
export class ToolValidator {
  ...
  static validateParameterSchema(schema: ToolParameterSchema): ToolValidationResult {
    ...
    // Check if schema exists and has type
        ...
        if (typeof schema.properties !== 'object' || schema.properties === null) {
          ...
          for (const [propName, propSchema] of Object.entries(schema.properties)) {
            if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(propName)) {
              warnings.push(
                `Property name '${propName}' should start with a letter and contain only letters, numbers, and underscores`
              );
            }
